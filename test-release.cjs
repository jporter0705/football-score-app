...
for(const script of html.matchAll(/<script([^>]*)>([\s\S]*?)<\/script>/g)){
 const source=/src="weeks.js"/.test(script[1])?fs.readFileSync(__dirname+'/weeks.js','utf8'):script[2];
...
setImmediate(()=>{try{
 ...
 assert.match(html,/build v4\.6\.0/);assert.match(html,/\.preview-tools\[hidden\]\{display:none\}/);
